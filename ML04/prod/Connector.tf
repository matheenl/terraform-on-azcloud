...
resource "azurerm_virtual_network_peering" "peer-fe-be" {
  name                      = "${var.prefix}-${var.env}-vnetpeering-fe-be"
  resource_group_name       = azurerm_resource_group.rg-fe.name
  virtual_network_name      = module.vnet-fe.vnet_name
  remote_virtual_network_id = module.vnet-be.vnet_id
}

resource "azurerm_virtual_network_peering" "peer-be-fe" {
  name                      = "${var.prefix}-${var.env}-vnetpeering-be-fe"
  resource_group_name       = azurerm_resource_group.rg-be.name
  virtual_network_name      = module.vnet-be.vnet_name
  remote_virtual_network_id = module.vnet-fe.vnet_id
}

resource "azurerm_network_security_rule" "nsgrule-jbox-Be" {
  name                        = "${var.prefix}-${var.env}-nsg Rule to allow inbound rdp from Jbox"
  priority                    = 110
  direction                   = "Inbound"
  access                      = "Allow"
  protocol                    = "Tcp"
...
resource "azurerm_firewall_nat_rule_collection" "azfw-natrule" {
  name                = "${var.prefix}-${var.env}-NatRuleCollection-01"
